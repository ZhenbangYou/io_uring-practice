...
int main(int argc, char *argv[]) {
  ...
  int ret_of_init = io_uring_queue_init_params(queue_depth, &ring, &params);
  assert(ret_of_init == 0);
  ...
  int res_of_register =
      io_uring_register_files(&ring, socket_fds.data(), socket_fds.size());
  assert(res_of_register == 0);
  ...
  while (remaining_tasks > 0) {
    ...
    if (status == CONNECTING) {
      ...
      auto sqe = io_uring_get_sqe(&ring);
      io_uring_prep_send(sqe, id, GET_REQUEST.c_str(), GET_REQUEST.length(), 0);
      sqe->user_data = id;
      sqe->flags |= IOSQE_FIXED_FILE;
      io_uring_submit(&ring);
      ...
      auto sqe = io_uring_get_sqe(&ring);
      io_uring_prep_recv(sqe, id, buffers[id], BUFFER_SIZE, 0);
      sqe->user_data = id;
      sqe->flags |= IOSQE_FIXED_FILE;
      io_uring_submit(&ring);
